...
if "`i2'"=="i2" {
    ...
    if `error' di as error "Method of moments failed when mvmeta ran, so I^2 cannot be computed"
        ...
        local col6 _col(59)
        local col7 _col(70)
        local line4 as text _dup(49) "{c -}"
        local line5 as text _dup(58) "{c -}"
        local line7 as text _dup(78) "{c -}"
        ...
        forvalues r=1/`p' {
            di as text "`yvar_`r''" as result `col2' `bssdest`r'' `col3' `bssdlow`r'' `col4' `bssdupp`r'' `col5' `i2fmt' `i2est`r'' `col6' `i2fmt' `i2low`r'' `col7' `i2fmt' `i2upp`r''
        }
        di `line7'
        ...
        if "`droppedterms'"=="yes" di as text "Note: one or more CIs for I^2 were computed by dropping zero terms"
